      ...
      <b-col cols="12" class="mb-3 mb-md-1">
        ...
        <div class="notifications-list">
          <notification-card
            ...
            @handle-delete="handleDeleteNotification"
          />
          <b-button variant="primary" @click="loadNotifications" class="w-100 mb-4 notifications-list__btn-loading">
              <i class="fa-regular mr-2" :class="isLoadingNotifications? 'fas fa-spinner fa-spin' : 'fa-plus'"></i>
...
export default class Notifications extends Vue {
  ...
  newNotifications: any = [
    {
      ...
      description: 'Lorem ipsum dolor sit amet consectetur adipisicing elit. Id omnis sequi dolorem assumenda saa sd epe amet perspiciatis. Cupiditate incidunt dolorum pariatur, quisquam obcaecati ratione odio eveniet ',
    },
    {
      id: 2,
      status: 0,
  ...
  async handleDeleteNotification(notification:any){ 
    const alertResult = await alertActionButton('Eliminar Notificación','¿Seguro que desea eliminar la notificación?', 'Rechazar', 'error')
    if(alertResult){
      ...
      const timeOut =setTimeout(() => {
        ...
        if(notification.status == 0){
          index = this.newNotifications.findIndex((not:any) => not.id == notification.id)
          this.newNotifications.splice(index,1)
        }else{
          index = this.oldNotifications.findIndex((not:any) => not.id == notification.id)
  ...
  handleCheckNotification(id:number){
    alertLoading()
